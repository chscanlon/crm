<div>
    ...
    <x-layout.section.container>
        <x-layout.section.header>Select File to Import</x-layout.section.header>
    ...
    Rows Imported : {{ $rowsStaged }} <br/>
    New Customers : {{ count($newCustomers) }} <br/>
    Deleted Customers : {{ count($deletedCustomers) }} <br/>
    Total Customers : {{ count($deletedCustomers) }} <br/>
        ...
        <div class="my-4">
            ...
            <table class="border table-auto">
                ...
                <tbody>
                    ...
                    @foreach ($deletedCustomers as $key => $value)
                    <tr>
                        <td> {{$deletedCustomers[$key]['timely_customer_id']}} </td>
                        <td> {{$deletedCustomers[$key]['first_name']}} </td>
                        <td> {{$deletedCustomers[$key]['family_name']}} </td>
                        <td> {{$deletedCustomers[$key]['sms']}} </td>
                        <td> {{$deletedCustomers[$key]['email']}} </td>
